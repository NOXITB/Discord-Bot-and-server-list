...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[upcordlist.noxitb.repl.co/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[upcordlist.noxitb.repl.co/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    serverClient.user.setPresence({ activity: { type: 'WATCHING', name: '-bump & -vote | servers' }, status: "dnd" });
});
