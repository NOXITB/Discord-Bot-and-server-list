...
exports.run = async (client, message, args) => {
	...
	if(!findServer) return message.channel.send(
		"This server was not found in our list.\nAdd your server: https://list.upcord.tk/server/add"
	);
    ...
    .setStyle("url")
    .setURL("https://list.upcord.tk/server/"+message.guild.id)
    ...
    message.channel.send({ embed: controlEmbed, buttons: [ sorgu, sorgu2, sorgu3 ].sort(() => Math.random()-0.5) }).then(async msg => {
		...
		const collector = await msg.createButtonCollector(filter, { time: 60000 });
		  collector.on('collect', async b => {
            ...
            if (cooldown - (Date.now() - lastDailyy) > 0)return msg.delete().then(await msgError('This command is used only once every 60 minutes.', { channel: message.channel }));
...
function msgError(msg, { channel }) {
    ...
    .setFooter('https://upcordlist.noxitb.repl.co/servers')
    .setDescription(msg)
    .setColor("RED")
    )
}