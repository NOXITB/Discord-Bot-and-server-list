...
InlineLexer.prototype.outputLink = function(cap, link) {
  ...
  if (cap[0].charAt(0) !== '!') {
    result = this.renderer.link(href, title, this.output(cap[1]));
  } else {
    var params;
    if (cap[4]) {
      params = {};
      var rawParams = cap[4].split(':');
      var regexp = /^(\d+(?:x\d+)?)$/;
      var numParams = 2;
      if (rawParams.length < numParams) {
        numParams = rawParams.length;
      }
      for (var i = 0; i < numParams; i++) {
        var param = rawParams[i].replace(/^\s+|\s+$/, '');
        if (param) {
          // size
          if (!params.size) {
            ...
            if (matches) {
              ...
              params.size = {};
              params.size.width = parseInt(size[0]);
              if (size[1]) {
                params.size.height = parseInt(size[1]);
              }
              continue;
            }
          }
    ...
    result = this.renderer.image(href, title, escape(cap[1]), params);
  }
...
Renderer.prototype.image = function(href, title, text, params) {
